...
namespace TFOOL.Enemies.AI
{
    public static class AIUtility
    {
        ...
        {
            // Only await as long as the current animation.
            float timer = AnimationUtility.GetCurrentAnimationDuration(enemy.Animator);
            while (enemy.AnimationSignal != signalName && !ct.IsCancellationRequested && timer > 0)
            {
